...
function updateFinalTotal() {
  ...
  if (vacRes && vacRes.style.display !== 'none') {
    ...
    const text = vacRes.innerText.replace(/\s+/g, ' ');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // note 요소가 있을 때만 텍스트 세팅 (없으면 에러 안 나게)
  if (note) {
    note.textContent =
      ' (수당 기준일: ' +
  ...
  function calcVacation() {
    ...
    // 자동모드일 때 직종 기준으로 기본급/정액급식비 다시 보정
  ...
  // 초기 수당 입력칸: 기본은 targetJobs 중 첫 직종 기준
